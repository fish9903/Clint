...
export function activate(context: vscode.ExtensionContext) {
	...
	function startChecking() {
		...
		let decorate: vscode.DecorationOptions[] = [];
	...
	// if statement
	function assignmentInIfstatement(decorate: vscode.DecorationOptions[]) {
		let regEx = /if\([^=!<>\r\n]*=[^=\r\n]+[^\r\n]*\)/g;
	...
	// while statement
	function assignmentInWhilestatement(decorate: vscode.DecorationOptions[]) {
		let regEx = /while\([^=!<>\r\n]*=[^=\r\n]+[^\r\n]*\)/g;
		let hoverMessage = '(EXP45-C)Recommend to change \'=\' to \'==\'';
